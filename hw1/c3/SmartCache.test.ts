...
async function testBasicOperations() {
  ...
  const cache = new SmartCache<string, number>({ maxSize: 10 });
...
async function testTTLExpiration() {
  ...
  const cacheWithDefaultTTL = new SmartCache<string, string>({ 
    maxSize: 10, 
  ...
  const cache = new SmartCache<string, string>({ maxSize: 10 });
...
async function testPersistence() {
  ...
  const cache1 = new SmartCache<string, { name: string; value: number }>({ 
    maxSize: 10, 
    persist: storageKey 
  });
  ...
  const cache2 = new SmartCache<string, { name: string; value: number }>({ 
    maxSize: 10, 
    persist: storageKey 
  });
  ...
  const cache3 = new SmartCache<string, string>({ 
    maxSize: 10, 
    defaultTTL: 100,
  ...
  const cache4 = new SmartCache<string, string>({ 
    maxSize: 10, 
    defaultTTL: 100,
    persist: 'test-cache-ttl' 
  });
...
async function testStressTest200Entries() {
  ...
  const cache = new SmartCache<string, { id: number; data: string }>({ 
    maxSize: 50,
  ...
  const persistentCache = new SmartCache<string, number>({ 
    maxSize: 100,
  ...
  const restoredCache = new SmartCache<string, number>({ 
    maxSize: 100,
    persist: 'stress-test-cache'
  });
  ...
  const mixedCache = new SmartCache<string, string>({ maxSize: 75 });
